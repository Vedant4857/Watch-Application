...
import com.school.erp.watch.data.StudentLeave
...
fun StudentLeavesScreen(
    ...
    val leavesState by viewModel.studentLeaves.collectAsStateWithLifecycle()
                ...
                ScalingLazyColumn(
                    ...
                    if (list.isEmpty()) {
                        ...
                        items(list.size) { index ->
                            StudentLeaveItem(
                                leave = list[index],
                                onApprove = { viewModel.handleStudentLeaveStatus(it.id, "approved") },
                                onDisapprove = { viewModel.handleStudentLeaveStatus(it.id, "disapproved") }
                            )
                        }
                    }
...
@Composable
fun StudentLeaveItem(
    leave: StudentLeave,
    onApprove: (StudentLeave) -> Unit,
    onDisapprove: (StudentLeave) -> Unit
) {
    Card(
        onClick = {},
        enabled = false,
        ...
        Column(
            ...
            Text(
                text = "${leave.leaveType}: ${leave.reason}",
                color = MutedGray,
                fontSize = 12.sp
            )
